...
def create_cnn_dataloaders(X: list, y: list, batch_size: int, seed: int = 7777777,
                           return_class_weights: bool = True, verbose: bool = False)\
        -> tuple[DataLoader, DataLoader, DataLoader] | tuple[DataLoader, DataLoader, DataLoader, torch.FloatTensor]:
    """
    Splits the data for train, validation, and test datasets.
    ...
    for signal, labels in zip(X, y):
        for window, label in zip(signal, labels):
            window = np.array(window).T
            X_expanded.append(window)
            y_expanded.append(label)

    X_train, X_val, y_train, y_val = train_test_split(X_expanded, y_expanded, test_size=0.175, random_state=seed)
    X_train, X_test, y_train, y_test = train_test_split(X_train, y_train, test_size=0.175, random_state=seed)
